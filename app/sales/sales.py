...
class SalesRepository:
    ...
    def _init_db(self):
        with self._get_connection() as conn:
            ...
            """)
            # 既存(稼働中)のleadsテーブルには無い可能性があるため、マイグレーションで補う。
            if pg:
                cursor.execute("ALTER TABLE leads ADD COLUMN IF NOT EXISTS callback_url TEXT")
            else:
                try:
                    cursor.execute("ALTER TABLE leads ADD COLUMN callback_url TEXT")
    ...
    async def create_lead(self, client_id: str, source: str, notes: str = "", callback_url: str = "") -> None:
        def _execute():
            with self._get_connection() as conn:
                cursor = conn.cursor()
                ph = "%s" if self.use_postgres else "?"
                cursor.execute(f"""
                INSERT INTO leads (client_id, source, stage, notes, callback_url)
                VALUES ({ph}, {ph}, 'lead', {ph}, {ph})
                """, (client_id, source, notes, callback_url or None))
                conn.commit()
        await asyncio.to_thread(_execute)

    async def update_lead_callback_url(self, client_id: str, callback_url: str) -> None:
        """
        Concierge経由で後からcallback_urlを教えてもらった場合に、既存リードへ反映する。
        直近のリード1件を対象にする(get_lead_by_clientと同じ選び方)。
        """
        def _execute():
            with self._get_connection() as conn:
                cursor = conn.cursor()
                ph = "%s" if self.use_postgres else "?"
                cursor.execute(f"""
                UPDATE leads SET callback_url = {ph}, updated_at = CURRENT_TIMESTAMP
                WHERE client_id = {ph}
